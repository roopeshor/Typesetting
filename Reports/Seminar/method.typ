...
Hardware components and ground reflections create direct current (DC) interference, which shows up as clutter near the zero-frequency point. In case of VORTRAD which adopts a zero-intermediate-frequency RF
transceiver architecture, produces  signal containing
a large dc power, and ground clutter with a small frequency offset. The paper doesn't mention a particular clutter removal method.
...
$ W_1 = (T_0 - alpha d_f, T_0 + alpha d_f) $
...
$ M_1 = (sum_(i=1)^(n-1) cal(F)lr((V'_(C i))))/(n-1) $
