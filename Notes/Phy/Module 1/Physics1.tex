\documentclass[12pt, a4paper]{article}
\input{../../imports.tex}
\input{../../tikz-shapes.tex}
\ExplSyntaxOn
\cs_gset_eq:NN \tbl_save_outer_table_cols: \scan_stop:
\ExplSyntaxOff

\begin{document}
\chapterheader{1}{Lasers \& Fibre optics}

\input{laser.tex}

\input{spontaneous-stimulated-emission.tex}

\input{einstein-coefficients.tex}

\input{population-inversion.tex}

\subtopic{Metastable state}
The atoms excited will undergo spontaneous emission and attain popalation inversion. These atoms reach a intermediate state between the transitioning states which has more life time than higher energy state. This state is known as \textbf{metastable state}. From here, atoms undergo stimulated emission and lasing action takes place. (In two level systems, lasing action doesn't takes place because of lack of metastable state)

\input{representing-transitions.tex}

\input{purity-of-spectral-lines.tex}

\vspace{1.5cm}

\input{questions-1.tex}

\input{fabrication-of-lasers.tex}

\topic{Different Lasers}

\input{he-ne-laser.tex}


\input{ruby-laser.tex}

\vspace{1.5cm}

\input{questions-2.tex}

\newpage

\topic{Fibre Optics}
Fibre optics deals with transmission of information using light (which has minimal energy loss). Total internal reflection is the principle behind it.

\input{tir.tex}

\input{structure-of-foc.tex}

\input{classification-of-foc_light-propagation.tex}

\vspace{1.5cm}
\input{numerical-aperture.tex}

\vspace{1.5cm}
\input{questions-3.tex}

\end{document}